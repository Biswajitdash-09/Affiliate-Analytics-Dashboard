...
export async function getDb() {
  ...
  // Initialize indexes only once using a promise to avoid race conditions
  if (!global._mongoIndexesPromise) {
    global._mongoIndexesPromise = initializeIndexes(db);
  }

  // Wait for indexes to be ready if needed, or just let it run in background
  // For better stability, we'll let it run in the background but it only runs ONCE now.
